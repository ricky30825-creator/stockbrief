...
- ticker: 미국 종목은 심볼(예: AAPL). 한국 종목의 6자리 코드는 100% 확실할 때만 쓰고 조금이라도 불확실하면 null로 둔다(틀린 코드는 금지). market은 "KR" 또는 "US".
...
def analyze_transcript(channel: str, title: str, transcript: str) -> dict:
    """자막을 분석해 {"summary", "opinions", "usage"} 반환. 실패 시 1회 재시도."""
    prompt = PROMPT.format(channel=channel, title=title, transcript=transcript)
    last_err = None
    for _ in range(2):
        try:
            raw, usage = _run_claude(prompt)
...
def _run_claude(prompt: str) -> tuple[str, dict]:
    # --max-turns 1: 도구 사용 등 에이전트 동작 차단, 순수 텍스트 1턴 응답 강제
    proc = subprocess.run(
...
def parse_cli_envelope(stdout: str) -> tuple[str, dict]:
    """claude -p --output-format json 응답에서 (본문 텍스트, 토큰 사용량)을 꺼낸다."""
    env = json.loads(stdout)
    if env.get("is_error") or not env.get("result"):
        raise ValueError(f"claude 응답 오류: {str(env)[:300]}")
    u = env.get("usage") or {}
    usage = {
        ...
        "cost_usd": env.get("total_cost_usd") or 0,
    }
        ...
        opinions.append(
            {
                ...
                # 한국 6자리 코드는 LLM이 자주 틀려서 표시하지 않는다 (미국 심볼은 신뢰 가능)
                "ticker": (op.get("ticker") or None) if market == "US" else None,
                "market": market,
                "stance": op["stance"],
